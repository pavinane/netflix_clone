...
const FavouriteButton:React.FC<FavouriteButtonProps> = ({movieId}) => {
    ...
    const toggleFavourites = useCallback(async () => {
        ...
        const updateFavouriteIds = response?.data?.favouriteId;

        mutate({
            ...currentUser,
            favouriteId:updateFavouriteIds
        });
         mutateFavourites();
...
return(
    ...
    className=" cursor-pointer group/item w-6 h-6  lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center
